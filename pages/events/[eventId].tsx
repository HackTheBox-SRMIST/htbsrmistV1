...
// Simple fallback if HexBackground isn't an imported component
const HexBackground = () => (
    <div className="absolute inset-0 opacity-10 pointer-events-none"
        style={{ backgroundImage: `url("data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M30 0l25.98 15v30L30 60 4.02 45V15z' fill-rule='evenodd' stroke='%239FEF00' fill='none'/%3E%3C/svg%3E")` }}
    />
);
...
const Event: NextPage<EventsPageProps> = ({ events }) => {
    ...
    return (
        <>
                ...
                <div className="mx-auto w-[90%] max-w-7xl font-share-tech px-6">
                    <div className=" rounded-2xl flex flex-col md:flex-row">
                        ...
                        <div className="md:w-1/2 w-full md:mr-16 text-white flex flex-col items-center justify-center p-6 space-y-5">
                            ...
                            <div className="loc bg-[#141D2B] text-white flex flex-row items-center space-x-4 p-4 w-full md:w-96 rounded-xl">
                                <div className="lgo flex items-center justify-center">
                                    <img src="/locationLogo.svg" alt="Location" className="h-6 w-6 md:h-8 md:w-8" />
                                </div>
                                <div className="txt w-full">
                                    <p className="text-xl md:text-2xl font-semibold break-words">{event?.venue}</p>
                                </div>
                            </div>
                            ...
                            {hasExternalRegistration ? (
                                <a
                                    ...
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className={`bg-htb-green w-full md:w-80 text-node-black font-bold text-center py-3 text-2xl rounded-2xl ${!isRegistrationActive ? "pointer-events-none opacity-60" : ""}`}
                                >
                                    Register Now
                                </a>
                            ) : (
                                <button
                                    className={`bg-htb-green w-full md:w-80 text-node-black font-bold text-center py-3 text-2xl rounded-2xl ${!isRegistrationActive ? "opacity-60" : ""}`}
                                    disabled={!isRegistrationActive}
                                    onClick={handlerReg}
                                >
                                    Register Now
                            ...
                            <Modal className="bg-htb-green" closeButton blur open={visibleReg} onClose={closeHandlerReg}>
                                <Modal.Body className="flex justify-center items-center font-mono">
                                    <ToastContainer />
                                    <p className="text-3xl font-bold max-md:text-2xl">Registration Form</p>
                                    <p>Please enter your Details</p>
                                    <form onSubmit={submitHandler} className=" gap-5 flex-col flex w-full">
                                        <Input required type="text" name="name" clearable bordered fullWidth color="primary" size="lg" placeholder="Name" maxLength={21}
                                            status={nameError ? "error" : "default"}
                                            onChange={(e) => {
                                                const value = e.target.value;
                                                if (value.length > 20) {
                                        ...
                                        />
                                        <Input required type="number" name="phn" clearable bordered fullWidth color="primary" size="lg" placeholder="Phone Number"
                                            status={phoneError ? "error" : "default"}
                                            onChange={(e) => {
                                                const value = e.target.value;
                                                if (value.length > 11) e.target.value = value.slice(0, 11);
                                                setPhoneError(value.length !== 10);
                                                if (value.length === 11) Toast(false, "Phone Number cannot exceed 10 digits");
                                            }}
                                        />
                                        <Input required type="text" name="usn" clearable bordered fullWidth color="primary" size="lg" placeholder="Registration Number" />
                                        <Input required type="email" name="email" clearable bordered fullWidth color="primary" size="lg" placeholder="Email"
                                        ...
                                        />
                                        <Input required type="text" name="dept" clearable bordered fullWidth color="primary" size="lg" placeholder="Department" />
                                        ...
                                        <p className="text-center font-extra-bold">Check us out on <a href="https://www.meetup.com/chennai-in/" className=" text-black text-xl hover:text-[#F74160] font-bold">MEETUP</a></p>
                                        <button type="submit" className="w-full bg-htb-green/50 hover:bg-htb-green py-2 font-normal text-lg rounded-lg" disabled={loadingSubmit}>
            ...
            <div className="speaker mt-10 font-share-tech text-center">
                <p className="text-htb-green text-3xl md:text-6xl font-bold">Know Our Guest</p>
                <div className={`spkr mt-8 px-4 md:px-16 lg:px-32 grid gap-6 ${(event?.speakers_details?.length ?? 0) <= 2 ? "grid-cols-1 sm:grid-cols-2 justify-center" : "grid-cols-1 sm:grid-cols-2 lg:grid-cols-3"}`}>
                    {event?.speakers_details?.map((guest, index) => (
                        <div key={index} className={`flex flex-col items-center bg-[#141D2B] hover:bg-[#1f2c42] rounded-xl border-2 border-htb-green p-6 shadow-lg w-full max-w-xs mx-auto ${!guest.image ? "justify-center h-full" : ""}`}>
                            {guest.image && (
                                <div className="w-full h-48 rounded-xl overflow-hidden flex items-center justify-center">
                                    <Image src={guest.image} alt={guest.name} width={250} height={250} className="object-cover rounded-xl" />
                                </div>
                            )}
                            <div className="speakerInfo text-center mt-4">
                                <p className="text-lg md:text-xl font-bold text-htb-green">{guest.name}</p>
                                <p className="text-sm md:text-base text-white px-4">{guest.designation}</p>
                            </div>
                        </div>
                    ))}
                </div>
            ...
            <div className="prereq font-share-tech mb-10">
                <p className="text-htb-green text-3xl sm:text-4xl md:text-5xl font-bold text-center my-6">Pre-Requisites</p>
                <div className="text-white px-4 sm:px-8 md:px-16 lg:px-64 text-xl sm:text-2xl md:text-3xl font-medium">
                    <ul className="list-disc list-inside">
                        {event?.prerequisites?.map((item, index) => (
                            <li key={index}>{item}</li>
            ...
            {visible ? (
                <div className="fixed inset-0 z-[100] text-white flex justify-center items-center backdrop-blur-xl">
                    <div className="w-[90%] lg:w-[500px] bg-white text-black p-7 rounded-3xl flex flex-col gap-5 relative ">
                        <ToastContainer />
                        <div className="absolute top-5 right-5 w-7 hover:cursor-pointer hover:text-red-600" onClick={closeHandler}>
                            <GiCrossMark className="w-full h-full" />
                        </div>
                        <p className="text-xl max-md:text-base">Please enter your registered E-Mail</p>
                        <Input type="email" clearable bordered fullWidth color="primary" size="lg" placeholder="Email" onChange={(event: any) => setEmail(event.target.value)} />
                        {certificate ? (
                            <a className="w-full bg-htb-green/50 hover:bg-htb-green py-2 font-normal rounded-full p-8 text-xl text-center" href={certificate} download="Certificate.png">Download Now</a>
                        ) : (
                            <div className="flex space-evenly justify-center">
                                <button
                                    onClick={fetchCertificate}
                                    disabled={loadingCertificate || type === "Please Select..."}
                                    className={`w-1/2 bg-htb-green hover:bg-htb-green/50 py-1 rounded-l-[20px] font-normal p-8 text-xl max-md:text-sm ${loadingCertificate ? "opacity-50 cursor-not-allowed" : ""}`}
                                >
                                    {loadingCertificate ? "Generating..." : "Generate Certificate"}
                                </button>
                                <div className=" max-md:w-2/3 text-xl max-md:text-lg mt-0 border-l-[1px] border-l-black">
                                    <Select autoFocus hideSelectedOptions={true} isClearable={false} isSearchable={false} placeholder={type}
                                        theme={(theme) => ({ ...theme, colors: { ...theme.colors, neutral50: "#000000" } })}
                                        tabSelectsValue={false} name="preference1" className="text-black w-full rounded-full" options={options} onChange={changeType} styles={styles}
                                    />
                                </div>
                            </div>
                        )}
